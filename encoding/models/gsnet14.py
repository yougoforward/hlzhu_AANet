...
__all__ = ['gsnet14', 'get_gsnet14']


class gsnet14(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(gsnet14, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = gsnet14Head(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class gsnet14Head(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(gsnet14Head, self).__init__()
...
class gs_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        gp = self.gap(x)
        # psaa
        y = torch.cat((feat0, feat1, feat2, feat3, gp.expand(n, c, h, w)), 1)
        out = self.project(y)
        out = self.pam0(out)
        return out, gp

def get_gsnet14(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = gsnet14(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
