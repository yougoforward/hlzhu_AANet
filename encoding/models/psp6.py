...
class PSP6(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(PSP6, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = PSP6Head(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class PSP6Head(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(PSP6Head, self).__init__()
...
def get_PSP6(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = PSP6(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
