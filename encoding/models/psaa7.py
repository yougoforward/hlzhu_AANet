...
class psaa7_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b4 = nn.Sequential(
            ...
            PAM_Module(512, 512 // 4, 512, out_channels, norm_layer)
        )
        ...
        self.skip_conv = nn.Sequential(nn.Conv2d(2*out_channels, out_channels, 1, padding=0, bias=False),
                                       norm_layer(out_channels),
                                       nn.ReLU(True))
    ...
    def forward(self, x):
        ...
        # out = self.pam(out)
        gap =self.gap(x)
        out = self.skip_conv(torch.cat([gap, out], dim=1))
