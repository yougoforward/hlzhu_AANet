...
__all__ = ['gsnet13', 'get_gsnet13']


class gsnet13(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(gsnet13, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)

        self.head = gsnet13Head(2048, nclass, norm_layer, se_loss, jpu=kwargs['jpu'], up_kwargs=self._up_kwargs)
        if aux:
...
class gsnet13Head(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, se_loss, jpu=False, up_kwargs=None,
                 atrous_rates=(12, 24, 36)):
        super(gsnet13Head, self).__init__()
    ...
    def forward(self, x):
        feat_sum, gap_feat = self.aa_gs(x)
...
def gsConv(in_channels, out_channels, atrous_rate, norm_layer):
    block = nn.Sequential(
...
class gsPooling(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(gsPooling, self).__init__()
...
class gs_Module(nn.Module):
    def __init__(self, in_channels, out_channels, atrous_rates, norm_layer, up_kwargs):
        ...
        self.b0 = nn.Sequential(
            ...
            norm_layer(out_channels),
            nn.ReLU(True))
        self.b1 = gsConv(in_channels, out_channels, rate1, norm_layer)
        self.b2 = gsConv(in_channels, out_channels, rate2, norm_layer)
        self.b3 = gsConv(in_channels, out_channels, rate3, norm_layer)
        ...
        self.project = nn.Sequential(nn.Conv2d(in_channels=in_channels + 5*out_channels, out_channels=out_channels,
    ...
    def forward(self, x):
        ...
        y2 = torch.cat((x, psaa_att_list[0] * feat0, psaa_att_list[1] * feat1, psaa_att_list[2] * feat2, psaa_att_list[3] *feat3, psaa_att_list[4]*gp.expand(n, c, h, w)), 1)
        out = self.project(y2)
        out = self.pam0(out+se*out)
        return out, gp

def get_gsnet13(dataset='pascal_voc', backbone='resnet50', pretrained=False,
                 root='~/.encoding/models', **kwargs):
    # infer number of classes
    from ..datasets import datasets
    model = gsnet13(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
