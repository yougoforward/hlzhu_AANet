...
class PSP5(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(PSP5, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = PSP5Head(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class PSP5Head(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(PSP5Head, self).__init__()
...
def get_PSP5(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = PSP5(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
...
def get_psp_resnet50_ade(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    >>> print(model)
    """
    return get_PSP5('ade20k', 'resnet50', pretrained, root=root, **kwargs)
...
class PyramidContext(nn.Module):
    ...
    def forward(self, x):
        ...
        ch_agg = torch.bmm(ch_attention, local_context.view(bs, self.out_channels,-1)).view(bs, self.out_channels,h,w)

        out = ps_agg + ch_agg + local_context
