...
class psp_att_noguide(BaseNet):
    def __init__(self, nclass, backbone, aux=True, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(psp_att_noguide, self).__init__(nclass, backbone, aux, se_loss, norm_layer=norm_layer, **kwargs)
        self.head = psp_att_noguideHead(2048, nclass, norm_layer, self._up_kwargs)
        if aux:
...
class psp_att_noguideHead(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, up_kwargs):
        super(psp_att_noguideHead, self).__init__()
...
def get_psp_att_noguide(dataset='pascal_voc', backbone='resnet50', pretrained=False,
    ...
    model = psp_att_noguide(datasets[dataset.lower()].NUM_CLASS, backbone=backbone, root=root, **kwargs)
    if pretrained:
        from .model_store import get_model_file
        model.load_state_dict(torch.load(
            get_model_file('psp_att_noguide_%s_%s'%(backbone, acronyms[dataset]), root=root)))
    return model

def get_psp_att_noguide_resnet50_ade(pretrained=False, root='~/.encoding/models', **kwargs):
    r"""psp_att_noguide model from the paper `"Context Encoding for Semantic Segmentation"
    ...
    Examples
    --------
    >>> model = get_psp_att_noguide_resnet50_ade(pretrained=True)
    >>> print(model)
    """
    return get_psp_att_noguide('ade20k', 'resnet50', pretrained, root=root, **kwargs)
