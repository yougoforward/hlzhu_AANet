...
__all__ = ['SegmentationLosses', 'PyramidPooling', 'JPU', 'Mean', 'JFPU', 'SegmentationMultiLosses', 'JSFPU', 'metric_SegmentationLosses', 'SegmentationGuideLosses', 'SegmentationLovaszLosses', 'City_SegmentationLosses']
...
class City_SegmentationLosses(CrossEntropyLoss):
    """2D Cross Entropy Loss with Auxilary Loss"""
    def __init__(self, se_loss=False, se_weight=0.2, nclass=-1,
                 aux=False, aux_weight=0.4, weight=None,
                 size_average=True, ignore_index=-1):
        super(City_SegmentationLosses, self).__init__(weight, size_average, ignore_index)
        self.se_loss = se_loss
        self.aux = aux
        self.nclass = nclass
        self.se_weight = se_weight
        ...
        self.criterion_ohem = OhemCrossEntropy2d(ignore_label=ignore_index, thresh=0.7, min_kept=100000, use_weight=True)
    ...
    def forward(self, *inputs):
        if not self.se_loss and not self.aux:
            ...
            # loss1 = lovasz_softmax_flat(*flatten_probas(pred1, target, self.ignore_index),
            #                           only_present=True)
            loss2 = self.criterion(pred2, target)
            return loss1 + self.aux_weight * loss2
        elif not self.aux:
            pred, se_pred, target = tuple(inputs)
            se_target = self._get_batch_label_vector(target, nclass=self.nclass).type_as(pred)
